...
- **Generated files are ephemeral:** `hwm sync` keeps generated files aligned with the active environment and builder.
  ...
  - `check` mode is no-write: `cabal`/`stack`/`nix` targets check presence, `hie` validates builder/component compatibility and reports errors when invalid, and `packages` runs validation-only checks.
