...
syncNixFile = do
  Config {..} <- asks config
  ops <- asks options
  benv@BuildEnvironment {buildGHC} <- getBuildEnvironment Nothing
  syncFile (optionsNix ops) (deriveFlakeNix benv cfgName buildGHC)
...
generateOverlay :: BuildEnvironment -> Name -> Version -> [Text]
generateOverlay benv projectName ghc =
...
deriveFlakeNix :: BuildEnvironment -> Name -> Version -> Text
deriveFlakeNix benv projectName version =
  T.unlines
    $ braces
      ( [ "description = \"A Haskell " <> projectName <> " workspace generated by HWM(Haskell Workspace Manager)\";",
          ...
          <> letBlock
            ( [ "supportedSystems = [ \"x86_64-linux\" \"aarch64-linux\" \"x86_64-darwin\" \"aarch64-darwin\" ];",
                ...
                <> generateOverlay benv projectName version
            )
