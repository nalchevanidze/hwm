...
deriveFlakeNix :: [(Pkg, [BuildEnvironment])] -> Context -> [Context] -> Text
deriveFlakeNix releases ctx@(projectName, benv) ctxs =
  T.unlines
    $ braces
      ( [ "description = \"A Haskell " <> projectName <> " workspace generated by HWM(Haskell Workspace Manager)\";",
          ...
          <> letBlock
            ...
            -- PACKAGES
            ( forAllSystems "packages" (nixPkgs <> artifactEngine) (genPackages (map fst releases) ctx ctxs)
                -- DEVSHELLS
...
-- PACKAGES
genPackages :: [Pkg] -> Context -> [Context] -> [Text]
...
genReleaseGroup :: Context -> [Pkg] -> [Text]
